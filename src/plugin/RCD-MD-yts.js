...
import ytdl from 'ytdl-core'; // Make sure to import ytdl-core
...
const song = async (m, Matrix) => {
  ...
  if (validCommands.includes(cmd)) {
          ...
          message: {
            ...
            interactiveMessage: proto.Message.InteractiveMessage.create({
              ...
              header: proto.Message.InteractiveMessage.Header.create({
                ...(await prepareWAMessageMedia({ image: { url: `https://files.catbox.moe/wuyr41.jpg` } }, { upload: Matrix.waUploadToServer })),
                title: ``,
                gifPlayback: true,
                subtitle: "",
    ...
    if (selectedVideo) {
      try {
        // Log the selected video ID for debugging
        console.log("Fetching details for video ID:", selectedVideo.videoId);

        const videoInfo = await ytdl.getBasicInfo(selectedVideo.videoId);

        // Check for age or privacy restrictions
        if (!videoInfo.videoDetails || videoInfo.videoDetails.isPrivate || videoInfo.videoDetails.age_restricted) {
          return m.reply('This video is private or age-restricted.');
        }
        ...
        m.reply('Error fetching video details. Please try again later.');
      }
    } else {
      m.reply('Selected video not found.');
    }
  }
};
